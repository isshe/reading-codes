...
// 缓冲区初始化函数。
// 参数buffer_end是缓冲区内存末端。对于具有16MB内存的系统，缓冲区末端被设置为4MB(buffer_end = 4MB)。
...
void buffer_init(long buffer_end)
{
	...
	while ( (b -= BLOCK_SIZE) >= ((void *) (h+1)) ) { 			// 前面是管理缓冲区的结构体，后面是分块的缓冲区
		h->b_dev = 0;							// 使用该缓冲块的设备号。
		h->b_dirt = 0;							// 脏标志，即缓冲块修改标志。
		h->b_count = 0;							// 缓冲块引用计数。
		h->b_lock = 0;							// 缓冲块锁定标志。
		h->b_uptodate = 0;						// 缓冲块更新标志（或称数据有效标志）。（？？？---isshe）
		h->b_wait = NULL;						// 指向等待该缓冲块解锁的进程。
		h->b_next = NULL;						// 指向具有相同hash值的下一个缓冲头。（具有相同hash值？？？）
		...
		h++;									// h指向下一新缓冲头位置。
		NR_BUFFERS++;							// 缓冲区块数累加。
		if (b == (void *) 0x100000)				// 若b递减到等于1MB，则跳过384KB，（？？？---isshe）
	...
	for (i=0;i<NR_HASH;i++)						// NR_HASH = 307 （？？？---isshe）
		hash_table[i]=NULL;
}	
