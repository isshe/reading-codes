...
/*
 * PCI的优点之一是：其支持寻找IRQ和每个设备所需的其他资源的探测方式(probing)相当优雅。
 * 探测方式分为：
 * * 静态：给定一个设备PCI ID，内核根据id_table向量查询出证券的PCI驱动程序(也就是pci_driver实例)。
 * * 动态：动态是指系统管理员可以新增ID的能力，而不是指ID本身可自行变动。
 * 		* 动态ID是在运行中的系统上配置的，因此只有当内核被编译成支持热插拔时才能使用。
 */
#ifndef LINUX_PCI_H
#define LINUX_PCI_H
...
/*
 * pci_dev: 描述PCI设备；每个PCI设备都会被分派一个pci_dev实例。
 * 如同网络设备都会被分派net_device实例一样。
 ...
 * 这个结构主要由函数指针组成。所有PCI设备都会使用这个结构。
 */
struct pci_driver {
	struct list_head	node;
	// 驱动程序名称
	const char		*name;
	// 一个ID向量，内核用于把一些设备关联到此驱动程序。
	const struct pci_device_id *id_table;	/* Must be non-NULL for probe to be called */
	// 当PCI层发现它正在搜寻设备驱动程序的设备ID与id_table匹配，就会调用此函数。
	// 新设备插入。
	int  (*probe)(struct pci_dev *dev, const struct pci_device_id *id);	/* New device inserted */
	// 当驱动程序从内核除名时，或者可热拔插设备被删除时，PCI层调用此函数。
	// 此函数为probe的配对函数，用于清理任何数据结构和状态。
	// 网络设备使用此函数来释放已分配的I/O端口和I/O内存、为设备除名、
	// 释放net_device以及其他有设备驱动程序在probe函数所分配的辅助数据结构。
	// 如果不是热拔插设备，则为NULL。
	void (*remove)(struct pci_dev *dev);	/* Device removed (NULL if not a hot-plug capable driver) */
	// 以下4个时挂起和恢复。当系统进入挂起模式以及重新继续时，PCI层会调用这些函数。
	// suspend: 停止设备出口队列，使得设备无法再传输。
	// resume: 重启出口队列，使得设备得以再次传输。
...
/*
 * pci_register_driver: 设备驱动程序用此宏进行驱动注册。
 * 此宏和后面的一个函数同名，所以到底是会调用哪个？怎么做到同名的？
 */
