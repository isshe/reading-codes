...
/* 注册时候用的pci_device_id：id_table;列出了驱动程序能处理的设备。
 * @PCI_VENDOR_ID_INTEL: 厂商ID是intel
 * @device_id: 设备ID。
 * @PCI_ANY_ID: 这两个是subxxxxx。前面两个已经能标识设备了，就不需要这两个了。
 * @PCI_CLASS_NETWORK_ETHERNET：设备所属的类+掩码。
 * @ich：是参数。
 */
...
// 注册设备到驱动。
// 场景：PCI层注册设备驱动，PCI层遍历所有关联的设备，逐一调用probe注册设备到驱动。
...
// 移除某个设备。
// 场景：卸载驱动，PCI层遍历所有与该驱动关联的设备，逐个调用remove。
...
static int e100_resume(struct pci_dev *pdev)
{
	...
	if (netiic *nic = netdev_priv(netdev);
...
module_init(e100_init_module);
module_exi	pci_unregister_driver(&e100_driver);
}

module_init(e100_init_module);
module_exit(e100_cleanmodule_init(e100_init_module);
m	pci_unregister_driver(&e100_driver);
}

module_init(e100_init_module);
module_exit(e100_cleanup_modumodule_exit(e100_clmodule_exit(e100_cleanup_module);
}

module_init(e100_init_module);
modstatic void __exit e100_cleanup_module(void)
{
	pci_unregister_driver(&e100_driver);
}

module_init(e100_init_module);
module_exit(e100_cleanup_mstatic void __exit e100_cleanup_module(void)
{
	pci_unregister_driver(&e100_driver);
}
